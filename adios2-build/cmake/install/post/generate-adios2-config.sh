...
CMAKE="/usr/bin/cmake"
SOURCE_DIR="/home/jlx/Projects/CAESAR_ALL/ADIOS2/cmake/install/post/adios2-config-dummy"
BUILD_DIR=$(mktemp -d)
pushd ${BUILD_DIR}
...
if [ 1 -eq 1 ]
then
  compiler_settings="$compiler_settings -DCMAKE_C_COMPILER=/usr/bin/mpicc"
fi
if [ 1 -eq 1 ]
then
  compiler_settings="$compiler_settings -DCMAKE_CXX_COMPILER=/usr/bin/mpicxx"
fi
if [ 1 -eq 1 ]
then
  compiler_settings="$compiler_settings -DCMAKE_Fortran_COMPILER=/usr/bin/mpif90"
fi
...
if [ ! -d "${adios2_DIR}" ]
then
  adios2_DIR="/home/jlx/Projects/CAESAR_ALL/ADIOS2/adios2-build/${PREFIX}/lib/cmake/adios2"
fi

if [ -n "/home/jlx/Projects/CAESAR_ALL/CAESAR_C/install/" ]
then
  export CMAKE_PREFIX_PATH="/home/jlx/Projects/CAESAR_ALL/CAESAR_C/install/:${CMAKE_PREFIX_PATH}"
fi

echo "Configuring adios2-config dummy CMake project"
if ! ${CMAKE} -G "Unix Makefiles" ${SOURCE_DIR} $compiler_settings ${EXTRA_CMAKE_ARGS}
then
  echo "Failed"
  exit 1
fi
...
cat \
  "/home/jlx/Projects/CAESAR_ALL/ADIOS2/adios2-build/cmake/install/post/adios2-config.pre.sh" \
  adios2.flags \
  "/home/jlx/Projects/CAESAR_ALL/ADIOS2/adios2-build/cmake/install/post/adios2-config.post.sh" > "${BINDIR}/adios2-config"

...
void WriteVectorOfStrings(char *buffer, size_t &pos,
  ...
  for (const auto &str : vec) WriteString(buffer, pos, str);
}
...
std::vector<std::string> ReadVectorOfStrings(const char *buffer, size_t &pos) {
  ...
  vec.reserve(count);
  for (uint64_t i = 0; i < count; ++i) vec.push_back(ReadString(buffer, pos));
  return vec;
}
...
void WriteVector2D(char *buffer, size_t &pos,
  ...
  for (const auto &v : vec2d) WriteVector(buffer, pos, v);
}
...
std::vector<std::vector<T>> ReadVector2D(const char *buffer, size_t &pos) {
  ...
  vec2d.reserve(outer);
  for (uint64_t i = 0; i < outer; ++i) vec2d.push_back(ReadVector<T>(buffer, pos));
  return vec2d;
}
...
void WriteVectorOfPairs(char *buffer, size_t &pos,
  ...
  uint64_t size = vec.size();
  WriteParameter(buffer, pos, size);
  for (const auto &p : vec) WritePair(buffer, pos, p);
}
...
std::vector<std::pair<T1, T2>> ReadVectorOfPairs(const char *buffer, size_t &pos) {
  ...
  vec.reserve(size);
  for (uint64_t i = 0; i < size; ++i) vec.push_back(ReadPair<T1, T2>(buffer, pos));
  return vec;
}
...
void WriteLBRCMetaData(char *buffer, size_t &pos, const LBRCMetaData &meta) {
  ...
  WriteParameter(buffer, pos, meta.encoded_nrmse);
  WriteVector(buffer, pos, meta.shape);
  WriteParameter(buffer, pos, meta.block_size[0]);
  WriteParameter(buffer, pos, meta.block_size[1]);
  WriteParameter(buffer, pos, meta.block_size[2]);
  WriteParameter(buffer, pos, meta.zstd_level);
  WriteParameter(buffer, pos, meta.quant_iter);
}
...
LBRCMetaData ReadLBRCMetaData(const char *buffer, size_t &pos) {
  ...
  meta.encoded_nrmse         = ReadParameter<double>(buffer, pos);
  meta.shape                 = ReadVector<int32_t>(buffer, pos);
  meta.block_size[0]         = ReadParameter<int64_t>(buffer, pos);
  meta.block_size[1]         = ReadParameter<int64_t>(buffer, pos);
  meta.block_size[2]         = ReadParameter<int64_t>(buffer, pos);
  meta.zstd_level            = ReadParameter<int>(buffer, pos);
  meta.quant_iter            = ReadParameter<int>(buffer, pos);
  return meta;
}
...
void WriteLBRCBlocks(char *buffer, size_t &pos, const std::vector<LBRCBlock> &blocks) {
  ...
  for (const auto &blk : blocks) {
    WriteParameter(buffer, pos, blk.b);
    WriteParameter(buffer, pos, blk.c);
    WriteParameter(buffer, pos, blk.t0);
    WriteParameter(buffer, pos, blk.t1);
    WriteParameter(buffer, pos, blk.h0);
    WriteParameter(buffer, pos, blk.h1);
    WriteParameter(buffer, pos, blk.w0);
    WriteParameter(buffer, pos, blk.w1);
    WriteParameter(buffer, pos, blk.step);
    WriteParameter(buffer, pos, blk.bit_count);
    WriteParameter(buffer, pos, blk.num);
...
std::vector<LBRCBlock> ReadLBRCBlocks(const char *buffer, size_t &pos) {
  ...
  for (uint64_t i = 0; i < n; ++i) {
    LBRCBlock blk;
    blk.b         = ReadParameter<int64_t>(buffer, pos);
    blk.c         = ReadParameter<int64_t>(buffer, pos);
    blk.t0        = ReadParameter<int64_t>(buffer, pos);
    blk.t1        = ReadParameter<int64_t>(buffer, pos);
    blk.h0        = ReadParameter<int64_t>(buffer, pos);
    blk.h1        = ReadParameter<int64_t>(buffer, pos);
    blk.w0        = ReadParameter<int64_t>(buffer, pos);
    blk.w1        = ReadParameter<int64_t>(buffer, pos);
    blk.step      = ReadParameter<double>(buffer, pos);
    blk.bit_count = ReadParameter<uint32_t>(buffer, pos);
    blk.num       = ReadParameter<int64_t>(buffer, pos);
...
size_t CompressCAESAR::Operate(const char *dataIn, const Dims &blockStart,
  ...
  if (type == DataType::Float) {
    data_tensor =
        torch::from_blob(const_cast<char *>(dataIn), sizes, torch::kFloat32).clone();
  ...
  if (original_shape_vec.size() >= 5 &&
      original_shape_vec[original_shape_vec.size() - 2] >= 2 &&
  ...
  if (!padding_info.was_padded) padded_5d = padded_5d.contiguous();
...
size_t CompressCAESAR::DecompressV1(const char *bufferIn, const size_t sizeIn,
  ...
  if (compressed_model != installed_model) {
    helper::Throw<std::runtime_error>(
        "Operator", "CompressCAESAR", "DecompressV1",
        "Model mismatch: data compressed with '" + compressed_model +
        "' but installed model is '" + installed_model + "'");
  }

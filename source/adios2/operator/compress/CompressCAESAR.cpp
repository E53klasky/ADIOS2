                               ...
                               const DataType type, char *bufferOut)
{
    ...
    if (model_type != m_Parameters.end())
    {
        ...
        if (model_name != "caesar_v") // make logic better we need caesar method of all models and yeah
        {
            helper::Throw<std::invalid_argument>("Operator", "CompressCAESAR", "Operate",
    ...
    std::cout << "\nTime taken for compression: " << duration.count() << " seconds" << std::endl;
    ...
    std::cout << "\nOriginal data size: " << sizeOut << " bytes" << std::endl;
...
size_t CompressCAESAR::DecompressV1(const char *bufferIn, const size_t sizeIn, char *dataOut)
{
    ...
    std::string device_str = DetectDevice();
    torch::Device device =
        (device_str == "cuda") ? torch::Device(torch::kCUDA) : torch::Device(torch::kCPU);
    ...
    torch::Tensor reconstructed = decompressor.decompress(
        comp.encoded_latents, comp.encoded_hyper_latents, batch_size, n_frame, comp);
    auto end_time = std::chrono::high_resolution_clock::now(); // for testing
    auto duration =
        std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time); // for testing
